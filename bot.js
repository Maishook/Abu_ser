...
${chalk.blue.italic('ℹ️ Connecting to WhatsApp...')}`);
    ...
    conn.on('open', async () => {
        ...
        if (config.AFPLK == 'amalser' || config.AFPLK == 'ABUOP' || config.AFPLK == 'vava' || config.AFPLK == 'Amalser') {
        //thanks to afnanplk
        console.log(
            chalk.green.bold('thanks for watching -key cofirmed-')
        );
         }
         else if (config.AFPLK !== 'amalser' || config.AFPLK !== 'ABUOP' || config.AFPLK !== 'vava' || config.AFPLK !== 'Amalser') {
         console.log(
	    ...
	    if (config.LANG == 'EN' || config.LANG == 'ML') {
                ...
                if (commits.total === 0) {
                    await conn.sendMessage(conn.user.jid, fs.readFileSync("./photo/amalser.png"), MessageType.image, { caption: `*ʜᴇʏ ʙʀᴏ ᴀʙᴜ sᴇʀ ᴡᴏʀᴋɪɴɢ ` + config.WORKTYPE + ` 🥂* \n\n*ᴛʜᴀɴᴋʏᴏᴜ ғᴏʀ ᴜsɪɴɢ ᴀʙᴜ_sᴇʀ_ʙᴏᴛ🥂*`});
                } else {
